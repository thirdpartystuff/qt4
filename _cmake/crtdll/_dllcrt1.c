...
//[NZ] p_atexit_fn __dllonexit (p_atexit_fn, p_atexit_fn**, p_atexit_fn**);
static CRITICAL_SECTION atexitcs;
static p_atexit_fn __dllonexit(p_atexit_fn func, p_atexit_fn** pbegin, p_atexit_fn** pend)
{
    p_atexit_fn *p;
    unsigned oldsize;

    EnterCriticalSection(&atexitcs);

    if ((oldsize = _msize(*pbegin)) <= (unsigned)((char*)(*pend) - (char*)(*pbegin))) {
        if ((p = (p_atexit_fn*)realloc((*pbegin), oldsize + 8 * sizeof(p_atexit_fn))) == NULL) {
            LeaveCriticalSection(&atexitcs);
            return NULL;
        }
        (*pend) = p + ((*pend) - (*pbegin));
        (*pbegin) = p;
...
#endif
      InitializeCriticalSection(&atexitcs);
  ...
  if ( (dwReason == DLL_PROCESS_ATTACH) && !bRet)
    {
      ...
      DeleteCriticalSection(&atexitcs);
    }
