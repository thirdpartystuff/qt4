...
__mingw_CRTStartup (void)
{
   ...
   */
  //[NZ] __cpu_features_init ();        /* Do we have SSE, etc.*/
  _fpreset ();                        /* Supplied by the runtime library. */

  /*
   * Set up __argc, __argv and _environ.
   */
  //[NZ] _mingw32_init_mainargs ();
   ...
   * for more details.
   */
  //[NZ] nRet = main (_argc, _argv, environ);
  STARTUPINFOA si;
  si.cb = sizeof(STARTUPINFOA);
  GetStartupInfoA(&si);
  nRet = WinMain(GetModuleHandleA(NULL), NULL, GetCommandLineA(), si.wShowWindow);
