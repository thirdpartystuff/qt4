...
{
    return mimeType.startsWith(s_text_plain)
...
{
    QVector<FORMATETC> formatics;
    if (mimeType.startsWith(s_text_plain) && (!mimeData || mimeData->hasText())) {
...
bool QWindowsMimeURI::canConvertFromMime(const FORMATETC &formatetc, const QMimeData *mimeData) const
{
    ...
    return getCf(formatetc) == CF_INETURL && mimeData->hasFormat(s_text_uri_list);
}
...
QVector<FORMATETC> QWindowsMimeURI::formatsForMime(const QString &mimeType, const QMimeData *mimeData) const
{
    QVector<FORMATETC> formatics;
    if (mimeType == s_text_uri_list) {
        if (!mimeData || canConvertFromMime(setCf(CF_HDROP), mimeData))
            formatics += setCf(CF_HDROP);
        if (!mimeData || canConvertFromMime(setCf(CF_INETURL), mimeData))
...
QVector<FORMATETC> QWindowsMimeHtml::formatsForMime(const QString &mimeType, const QMimeData *mimeData) const
{
    ...
    if (mimeType == s_text_html && (!mimeData || !mimeData->html().isEmpty()))
...
{
    return mimeType == s_text_html && canGetData(CF_HTML, pDataObj);
}
...
{
    QVector<FORMATETC> formatetcs;
    if ((!mimeData || mimeData->hasImage()) && mimeType == s_application_x_qt_image) {
...
QString QWindowsMimeImage::mimeForFormat(const FORMATETC &formatetc) const
{
    if (getCf(formatetc) == CF_DIB)
       return s_application_x_qt_image;
    return QString();
}

bool QWindowsMimeImage::canConvertToMime(const QString &mimeType, IDataObject *pDataObj) const
{
    if (mimeType == s_application_x_qt_image && canGetData(CF_DIB, pDataObj))
        return true;
    return false;
}
...
: QWindowsMime()
{
    outFormats.insert(QWindowsMime::registerMimeType(s_text_uri_list), s_text_uri_list);
    inFormats.insert(QWindowsMime::registerMimeType(s_text_uri_list), s_text_uri_list);
    outFormats.insert(QWindowsMime::registerMimeType(s_text_plain), s_text_plain);
    inFormats.insert(QWindowsMime::registerMimeType(s_text_plain), s_text_plain);
    outFormats.insert(QWindowsMime::registerMimeType(s_text_html), s_text_html);
    inFormats.insert(QWindowsMime::registerMimeType(s_text_html), s_text_html);
    outFormats.insert(QWindowsMime::registerMimeType(s_application_x_color), s_application_x_color);
    inFormats.insert(QWindowsMime::registerMimeType(s_application_x_color), s_application_x_color);
    outFormats.insert(QWindowsMime::registerMimeType(s_application_x_qt_image), s_application_x_qt_image);
    inFormats.insert(QWindowsMime::registerMimeType(s_application_x_qt_image), s_application_x_qt_image);
}
...
bool QBuiltInMimes::convertFromMime(const FORMATETC &formatetc, const QMimeData *mimeData, STGMEDIUM * pmedium) const
{
    if (canConvertFromMime(formatetc, mimeData)) {
        QByteArray data;
        if (outFormats.value(getCf(formatetc)) == s_text_html) {
...
{
    QVector<FORMATETC> formatetcs;
    if (!outFormats.keys(mimeType).isEmpty() && (!mimeData || mimeData->formats().contains(mimeType)))
...
#endif
            if (mimeType == s_text_html && preferredType == QVariant::String) {
                // text/html is in wide chars on windows (compatible with mozillia)
