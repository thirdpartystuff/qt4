...
BOOL CALLBACK enumCallback(HMONITOR hMonitor, HDC, LPRECT, LPARAM)
{
    ...
    BOOL res = (pfnGetMonitorInfoW ? pfnGetMonitorInfoW(hMonitor, &info) : pfnGetMonitorInfoA(hMonitor, &info));
    if (!res) {
...
void QDesktopWidgetPrivate::init(QDesktopWidget *that)
{
    ...
    if (pfnEnumDisplayMonitors && (pfnGetMonitorInfoA || pfnGetMonitorInfoW)) {
        screenCount = 0;
        pfnEnumDisplayMonitors(0, 0, enumCallback, 0);
      #ifndef QT_NO_DEBUG
        ...
        if (QSysInfo::WindowsVersion == QSysInfo::WV_NT
            ...
            || QSysInfo::WindowsVersion == QSysInfo::WV_32s)
            screenCount = 1;
        else
        ...
        if (SystemParametersInfo(SPI_GETWORKAREA, 0, &r, 0))
            ...
            qr = that->rect();
      #ifndef QT_NO_DEBUG
        qDebug("workarea = (%d, %d, %d, %d)", qr.left(), qr.top(), qr.right(), qr.bottom());
      #endif
